...
# IMPORTANT:
# This is NOT a reduced dataset.
# All original variables are kept.
# Selected variables are simply moved to the front.
OUTPUT_FILE = "./dataset/traffic/traffic_v4_partial.csv"
...
def main():
    ...
    print("=" * 70)
    print("Creating Traffic partial-training dataset")
    print("=" * 70)
    ...
    # --------------------------------------------------------
    # Create reordered FULL dataset
    # --------------------------------------------------------
    #
    # IMPORTANT:
    #
    # We DO NOT remove any variables.
    #
    # We ONLY reorder the original columns:
    #
    #   date
    #   selected variables
    #   remaining variables
    #   OT
    #
    # This allows the official iTransformer
    # partial-training implementation to use:
    #
    #   enc_in = N_SELECTED
    #
    # During training, the first N_SELECTED variables
    # are used.
    #
    # During inference, the complete dataset is used.
    # --------------------------------------------------------
    ...
    if len(df_v4.columns) != expected_columns:

        raise RuntimeError(
            "Column count changed unexpectedly. "
            f"Original={expected_columns}, "
            f"New={len(df_v4.columns)}"
        )
    ...
    if (
        df_v4.columns[1:1 + len(selected_features)].tolist()
        != selected_features
    ):

        raise RuntimeError(
            "Selected variables were not placed "
    ...
    with open(
        ...
        f.write(
            "Traffic partial-training variable selection\n"
        )
        ...
        f.write(
            f"Selected training variables: "
            f"{len(selected_features)}\n"
        )

        f.write(
            f"Remaining variables: "
        ...
        f.write(
            "\nSelected training variables "
            "(placed first in dataset):\n"
        )
